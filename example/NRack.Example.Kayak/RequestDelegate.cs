...
namespace NRack.Example.Kayak
{
    public class RequestDelegate : IHttpRequestDelegate
    {
        ...
        public RequestDelegate(ConfigBase config, IDictionary<string, object> context)
        {
            var builder = new Builder(config.ExecuteStart);
            AppDelegate appDel = (env, result, fault) =>
                                     {
                                         var resp = builder.Call(env);
                                         Func<ArraySegment<byte>, Action, bool> next = (resp[2]
                                     };
            this.appDelegate = appDel;
